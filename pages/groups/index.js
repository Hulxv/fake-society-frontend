...
export default function Groups({ groups, posts }) {
	return (
		<div
			className={
				"w-full flex flex-col space-y-4 items-center overflow-y-hidden"
			}>
			<div className={"w-full  "}>
				<div
					className={
						"shadow space-y-2 overflow-hidden  w-full bg-white rounded-xl py-4 flex flex-col items-center justify-center"
					}>
					<div className={"flex items-center justify-center px-2 w-full"}>
						<Search
							Component={<Button colorScheme={"blue"}>Join Group</Button>}
							taps={["groups"]}
						/>
					</div>
					<div
						className={
							"w-full space-x-6 flex overflow-x-auto py-4 pr-4 items-center "
						}>
						<div className={""}>
							<div className='flex w-20 flex-col px-4 box-content space-y-2 justify-center items-center'>
								<IconButton
									colorScheme={"blue"}
									icon={<HiPlus size='2.3em' />}
								/>
							...
							<div className={""} key={index}>
								<div className='flex flex-col space-y-1 items-center'>
									...
									<div className={"flex-col items-center flex"}>
										<h1>{group.name}</h1>
										<h6 className='opacity-40' style={{ fontSize: "10px" }}>
			...
			<div className={"w-full flex flex-col space-y-4"}>
				{posts.map((post, index) => {
					// For Testing
					const groups = ["Developers", "Memes Society", "Science Society"];
